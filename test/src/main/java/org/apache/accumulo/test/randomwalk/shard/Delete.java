...
public class Delete extends Test {
  ...
  public void visit(State state, Properties props) throws Exception {
    ...
    Entry<Key,Value> entry = Search.findRandomDocument(accumuloState, dataTableName, rand);
    if (entry == null)
      return;
    ...
    Insert.unindexDocument(accumuloState.getMultiTableBatchWriter().getBatchWriter(indexTableName), doc, docID, numPartitions);
    
    Mutation m = new Mutation(docID);
    m.putDelete("doc", "");
    
    accumuloState.getMultiTableBatchWriter().getBatchWriter(dataTableName).addMutation(m);
