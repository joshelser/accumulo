...
public class DeleteWord extends Test {
  ...
  public void visit(State state, Properties props) throws Exception {
    ...
    Scanner scanner = accumuloState.getConnector().createScanner(indexTableName, Authorizations.EMPTY);
    ...
    if (documentsToDelete.size() > 0) {
      ...
      BatchScanner bscanner = accumuloState.getConnector().createBatchScanner(docTableName, Authorizations.EMPTY, 8);
      bscanner.setRanges(documentsToDelete);
      
      BatchWriter ibw = accumuloState.getMultiTableBatchWriter().getBatchWriter(indexTableName);
      BatchWriter dbw = accumuloState.getMultiTableBatchWriter().getBatchWriter(docTableName);
